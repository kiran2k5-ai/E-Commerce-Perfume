...
const Admin = () => {
    ...
    try {
      if (activeTab === 'products') {
        ...
        const { data } = await axios.get(
          'https://e-commerce-perfume-backend.onrender.com/api/orders/all',
          { headers: { Authorization: `Bearer ${user.token}` } }
        )
        setOrders(data)
      } else if (activeTab === 'subscribers') {
        const { data } = await axios.get(
          'https://e-commerce-perfume-backend.onrender.com/api/newsletter/subscribers',
          { headers: { Authorization: `Bearer ${user.token}` } }
        )
        setSubscribers(data)
      }
  ...
  const updateOrderStatus = async (orderId, status) => {
    try {
      await axios.put(
        `https://e-commerce-perfume-backend.onrender.com/api/orders/${orderId}/status`,
        { status },
        { headers: { Authorization: `Bearer ${user.token}` } }
      )
      fetchData()
