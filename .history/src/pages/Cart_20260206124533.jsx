  ...
  return (
    <div >
      <div className='flex flex-row justify-center item-center gap-100 text-black '>
        ...
        <div className='flex flex-col mt-50 gap-5 justify-center items-center text-2xl font-light italic border border-grey-300 w-130 h-150 rounded-2xl'>
          <h1>{product.name}</h1>
          ...
          breeze through wildflower fields,<br />
           ...
           refreshing notes.</h2>
          <div >
            <div className='flex flex-row gap-5'>Qty <div className='w-20 h-10 border-2 pl-8 rounded'>{count}</div>
            <button className='' onClick={()=>setCount(count+1)}>+</button></div>
            <button className='' onClick={()=>setCount(coun+1)}>-</button></div>
          </div>
