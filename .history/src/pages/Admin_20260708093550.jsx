...
const Admin = () => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/product/${editProduct._id}/upload-image`, form, {
        headers: { Authorization: `Bearer ${user.token}`, 'Content-Type': 'multipart/form-data' }
      })
      // update image field with returned image path
      setEditFields(f => ({ ...f, image: res.data.image || res.data.image }))
  ...
  const deleteSubscriber = async (id) => {
    try {
      await axios.delete(`${API_URL}/api/newsletter/subscribers/${id}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      })
  ...
  const exportSubscribersCsv = () => {
    ...
    subscribers.forEach(s => rows.push([s.email, new Date(s.subscribedAt).toISOString()]))
  ...
  const sendEmailToAll = async () => {
    ...
    try {
      await axios.post(`${API_URL}/api/newsletter/subscribers/send`, { subject: emailSubject, message: emailBody, toAll: true }, {
        headers: { Authorization: `Bearer ${user.token}` }
      })
      setMessage('Emails queued/sent')
      setShowEmailModal(false)
      setEmailSubject('')
      setEmailBody('')
    } catch (error) {
      console.error(error)
      setMessage('Unable to send emails')
    } finally {
      ...
      <div className="container mx-auto px-4 py-8 lg:py-12">
        <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
          ...
          <div className="space-y-6">
            <div className="rounded-4xl bg-white/90 shadow-black p-6 backdrop-blur-md">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-3xl text-5xl italic uppercase text-pink-500">Admin summary</p>
                  ...
                  <div className="rounded-4xl bg-white p-6 shadow-xl overflow-x-auto">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex gap-3">
                        <button onClick={exportSubscribersCsv} className="rounded-2xl border px-3 py-2">Export CSV</button>
                        <button onClick={() => setShowEmailModal(true)} className="rounded-2xl bg-purple-600 px-3 py-2 text-white">Email all</button>
                      </div>
                    </div>
                    <table className="w-full text-left">
                          ...
                          <tr key={sub._id} className="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td className="p-4">
                              <div className="flex gap-2">
                                <button onClick={() => deleteSubscriber(sub._id)} className="rounded-2xl bg-red-500 px-3 py-2 text-white">Delete</button>
                                <button onClick={async () => {
                                  try {
                                    await axios.post(`${API_URL}/api/newsletter/subscribers/send`, { id: sub._id, subject: `Hello ${sub.email}`, message: 'Message from admin' }, { headers: { Authorization: `Bearer ${user.token}` } })
                                    setMessage('Email sent')
                                  } catch (e) { setMessage('Unable to send email') }
                                }} className="rounded-2xl border px-3 py-2">Email</button>
                              </div>
                            </td>
                          </tr>
                        ))}
      ...
      {showEmailModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-lg rounded-4xl bg-white p-6 shadow-2xl">
            <h3 className="text-xl font-semibold mb-4">Email all subscribers</h3>
            <div className="space-y-3">
              <div>
                <label className="text-sm font-medium">Subject</label>
                <input value={emailSubject} onChange={(e) => setEmailSubject(e.target.value)} className="w-full rounded-2xl border px-3 py-2" />
              </div>
              <div>
                <label className="text-sm font-medium">Message</label>
                <textarea value={emailBody} onChange={(e) => setEmailBody(e.target.value)} className="w-full rounded-2xl border px-3 py-2" rows={6} />
              </div>
              <div className="flex justify-end gap-3">
                <button onClick={() => setShowEmailModal(false)} className="rounded-2xl border px-4 py-2">Cancel</button>
                <button onClick={sendEmailToAll} disabled={emailLoading} className="rounded-2xl bg-purple-600 px-4 py-2 text-white">{emailLoading ? 'Sending...' : 'Send'}</button>
              </div>
            </div>
          </div>
        </div>
