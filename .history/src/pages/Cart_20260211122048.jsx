          ...
          >
            {related.map(item => (
              <div key={item._id} className="px-4">
                <div className="border rounded-xl p-4 hover:shadow-black transition bg-white">
                  <img
                    src={item.image}
                    alt={item.name}
                  ...
                  <button
                    className="mt-3 w-full px-5 py-2 text-sm bg-gradient-to-r from-pink-300 to-purple-600 text-white rounded-full hover:shadow-md transition"
                    onClick={() =>
                      navigate("/cart", {
                        state: { product: item }
                      })
