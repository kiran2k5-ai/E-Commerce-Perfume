...
const Chatbot = () => {
  ...
  const getRecommendations = async (prefs) => {
    try {
      ...
      const { data } = await axios.get('https://e-commerce-perfume-backend.onrender.com/api/product')
      ...
      if (prefs.gender && prefs.gender !== 'Unisex') {
        filtered = filtered.filter(p => 
          p.category === prefs.gender || p.category === 'Unisex'
        )
      }
      ...
      if (recommendations.length > 0) {
        addMessage('bot', `Based on your preferences, I recommend these ${recommendations.length} fragrances:`)
        ...
        addMessage('bot', 'Sorry, I couldn\'t find any fragrances matching your criteria. Try adjusting your preferences!')
      }
    } catch (error) {
