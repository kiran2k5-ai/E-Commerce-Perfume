...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    { type: 'bot', text: `Hello! I'm your friendly fragrance assistant — I'll ask a few quick questions and recommend a few perfumes you'll likely love.` }
  ])
  ...
  const processMessage = async (message) => {
    ...
    const startKeywords = ['help', 'find', 'recommend', 'hi', 'hello', 'start', 'restart', 'new']

    if (startKeywords.some(k => lower.includes(k)) || step === 'greeting') {
      setStep('scentFamily')
      ...
      const newPrefs = { ...preferences, budget: message }
      setPreferences(newPrefs)
      await getRecommendations(newPrefs)
      setStep('complete')
    } else if (step === 'complete') {
      if (lower.includes('restart') || lower.includes('new')) {
        handleRestart()
      } else if (lower.includes('more')) {
        // fetch another set (for now re-run same recommendation)
        await getRecommendations(preferences)
      } else {
        addMessage('bot', 'Type "restart" to start a new search, or "more" to see more recommendations.')
      }
    } else {
      addMessage('bot', 'I can help you find the perfect fragrance! Say "recommend" or "hi" to begin.')
    }
  }

  const getRecommendations = async (prefs) => {
    try {
      addMessage('bot', 'Great — let me find a few fragrances that match your choices...')
      ...
      if (prefs.budget) {
        ...
        filtered = filtered.filter(p => {
          ...
          const productPrice = parseFloat(raw.replace(/[^0-9.]/g, '')) || 0
      ...
      if (recommendations.length > 0) {
        ...
        addMessage('bot', 'Would you like to see more options or start a new search? You can add any recommendation to your cart.')
      } else {
        addMessage('bot', 'Sorry, I\'m couldn\'t find any fragrances. Please try again later!')
      }
    } catch (error) {
  ...
  const handleAddToCart = (product) => {
    const prod = { id: product.id, name: product.name, price: product.price, image: product.image }
    addToCart(prod, 1)
    addMessage('bot', `Added "${product.name}" to your cart.`)
  }

  const handleRestart = () => {
    setMessages([{ type: 'bot', text: `Sure — let's start over. What's your first preference?` }])
    setStep('greeting')
    setPreferences({})
    setInput('')
  }
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-96 h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col z-50">
          <div className="bg-gradient-to-r from-pink-400 to-purple-600 text-white p-4 rounded-t-2xl flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleRestart} className="text-sm bg-white/20 hover:bg-white/30 text-white px-3 py-1 rounded">Restart</button>
                ...
                {msg.type === 'product' && (
                  <div className="bg-gradient-to-br from-pink-50 to-purple-50 rounded-lg p-3 border border-purple-200">
                    <div className="flex gap-3">
                      {msg.text.image && (
                        <img src={msg.text.image} alt={msg.text.name} className="w-16 h-16 object-cover rounded" />
                      )}
                      <div className="flex-1">
                        <h4 className="font-semibold text-purple-900">{msg.text.name}</h4>
                        <p className="text-sm text-gray-600 mt-1 line-clamp-2">{msg.text.description}</p>
                        <p className="text-lg font-bold text-purple-600 mt-2">₹{msg.text.price}</p>
                        <div className="mt-2 flex gap-2">
                          <button onClick={() => handleAddToCart(msg.text)} className="px-3 py-1 bg-purple-600 text-white rounded">Add to cart</button>
                          <a href={`/product/${msg.text.id}`} className="px-3 py-1 border border-purple-300 text-purple-700 rounded">View</a>
                        </div>
                      </div>
                    </div>
                  </div>
